...
const Card:React.FC<props> = (props) => {
    let newprice=props.price-props.price*props.reduction/100 ;
    ...
    return ( 
        ...
        ">
            <img src={img.src} alt=""  className="h-[20vw] rounded-t-[20px] bg-cover w-full max-lg:h-[22vw] max-sm:h-[30vw] max-sm:rounded-t-[15px] " />
        <div className='px-[10px] py-2 max-[550px]:px-2 '>
        <div className="flex justify-between items-center ">
         <h1 className="text-[25px] font-bold text-[rgba(77,77,129,1)] max-xl:text-[22px] max-lg:text-[17px] max-sm:text-[14px] max-[550px]:text-[12px] ">{props.Name}</h1>
    
    <div className={`flex place-content-center rounded-[10px]  bg-[rgba(255,52,57,0.3)] p-[2px] px-2 max-lg:px-[6px] max-lg:rounded-none ${props.reduction!=0 ? 'visible' : 'invisible' } `}>
       { dir &&  <p className='text-[18px] text-[#E00409] max-xl:text-[16px] max-lg:text-[11px] max-sm:text-[10px] max-[550px]:text-[8px] ' >-{props.reduction}%</p> }
       { !dir &&  <p className='text-[18px] text-[#E00409] max-xl:text-[16px] max-lg:text-[11px] max-sm:text-[10px] max-[550px]:text-[8px] ' >%{props.reduction}-</p>} 
            </div>
         </div>
       { props.livraison &&  <div className='flex my-3 mt-2 place-content-center bg-[rgba(11,255,6,0.3)] w-[14vw] min-w-max px-2 rounded-[10px] 
       max-lg:rounded-[5px] max-sm:my-1 '>
         ...
         { !props.livraison &&  <div className='flex my-3 mt-2 place-content-center bg-[rgba(255,170,6,0.3)] w-[14vw] px-2
        ...
        { props.reduction!=0 &&  <h1  className=' text-[rgba(77,77,129,0.6)] translate-y-[10%] font-[600] line-through '>{newprice} {currency} </h1> }
         </span>
